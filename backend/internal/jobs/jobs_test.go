...
func TestNewLLMPentestTask(t *testing.T) {
	task := NewLLMPentestTask("org_123", "run_456", "https://api.example.com/chat", "sk-test", []string{"prompt_injection", "jailbreak"}, "gpt-4", "openai", "", "both", 8, 4, false)
...
func TestEnqueueLLMPentestTask(t *testing.T) {
	// Just verify it doesn't panic and returns nil (runs async)
	task := NewLLMPentestTask("org_123", "run_456", "https://api.example.com/chat", "sk-test", []string{"prompt_injection"}, "gpt-4", "openai", "", "both", 8, 4, false)
